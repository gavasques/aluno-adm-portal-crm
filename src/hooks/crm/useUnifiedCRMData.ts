...
export const useUnifiedCRMData = (filters: CRMFilters = {}) => {
  ...
  const fetchUnifiedLeadsData = useCallback(async (): Promise<LeadWithContacts[]> => {
    ...
    try {
      console.log('📊 [UNIFIED_CRM] Buscando leads para pipeline:', debouncedFilters.pipeline_id);
      ...
      // Agrupar contatos por lead de forma eficiente
      ...
      console.log('📊 [UNIFIED_CRM] Leads processados com filtros:', leadsWithContactsData.length);
