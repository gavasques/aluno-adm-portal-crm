...
import { Plus, X, Tag, Hash, Loader2 } from 'lucide-react';
import { useOptimizedCRMTags } from '@/hooks/crm/useOptimizedCRMTags';
import { toast } from 'sonner';
import ModernFloatingInput from './ModernFloatingInput';

interface OptimizedTagsSelectorProps {
  selectedTags: string[];
  onTagsChange: (tags: string[]) => void;
}

const OptimizedTagsSelector = React.memo(({ selectedTags, onTagsChange }: OptimizedTagsSelectorProps) => {
  const { tags, loading, createTag } = useOptimizedCRMTags();
  ...
  }, [newTagName, createTag, selectedTags, onTagsChange]);
    ...
    return (
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-gray-600">Carregando tags...</span>
        </div>
      </div>
    );
  }
...
OptimizedTagsSelector.displayName = 'OptimizedTagsSelector';

export default OptimizedTagsSelector;
