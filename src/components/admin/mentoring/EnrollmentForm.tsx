...
import { useMentoringQueries } from '@/features/mentoring/hooks/useMentoringQueries';
...
const EnrollmentForm = ({ onSubmit, onCancel, initialData, isLoading }: EnrollmentFormProps) => {
  const { useCatalogs } = useMentoringQueries();
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          <Tabs defaultValue="basico" className="w-full">
            <TabsList className="grid w-full grid-cols-3 mb-4">
              <TabsTrigger value="basico" className="flex items-center gap-2 text-xs">
                <User className="h-4 w-4" />
                Dados Básicos
              </TabsTrigger>
              <TabsTrigger value="mentoria" className="flex items-center gap-2 text-xs">
                                    ...
                                    <div className="flex items-center justify-between">
                                      <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-1">
                                          <span className="font-medium text-gray-900 text-sm">{catalog.name}</span>
                                      ...
                                      <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition-colors ml-3 ${
                                        field.value === catalog.id
