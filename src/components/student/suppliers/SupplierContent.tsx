...
export function SupplierContent({
  ...
  return <AnimatePresence mode="wait">
      {!selectedSupplier ? <motion.div key="supplier-list" initial={{
      opacity: 0,
      y: 20
    }} animate={{
      opacity: 1,
      ...
      delay: 0.1
          ...
          <Card className="overflow-hidden border-none shadow-lg bg-gradient-to-br from-white to-amber-50">
            <CardHeader className="bg-gradient-to-r from-black-500 to-orange-500 text-white">
            ...
            <CardContent className="p-0">
              {isLoading ? <motion.div className="p-6 space-y-4" initial={{
            opacity: 0
          }} animate={{
            opacity: 1
          }} transition={{
            ...
            duration: 0.4
                  ...
                  <SuppliersTable suppliers={paginatedSuppliers} isAdmin={false} onSelectSupplier={setSelectedSupplier} pageSize={pageSize} onPageSizeChange={setPageSize} currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} sortField={sortField} sortDirection={sortDirection} onSort={handleSort} />
                </motion.div>}
            </CardContent>
          </Card>
        </motion.div> : <motion.div key="supplier-detail" initial={{
      opacity: 0,
      scale: 0.95
    }} animate={{
      opacity: 1,
      ...
      duration: 0.4
    }} className="bg-white rounded-lg shadow-xl border border-amber-100 overflow-hidden">
          <SupplierDetail supplier={selectedSupplier} onBack={() => setSelectedSupplier(null)} onUpdate={handleUpdateSupplier} />
        </motion.div>}
    </AnimatePresence>;
}